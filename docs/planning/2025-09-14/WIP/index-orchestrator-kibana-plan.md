...
- [ ] Add central configuration keys for index naming and alias to `conf/` or a top-level config file (if not present). Example keys:
...
- [ ] Create `scripts/maintenance/reindex_orchestrator.py` with a CLI that accepts:
...
- [ ] Script should load project config and logging setup.
...
7) Validation & sample evaluation (30–60 minutes)
- [ ] Implement `validate_index(index_name, validation_path, sample_limit)` which runs the repository's retrieval pipeline pointed at `index_name` using `sample_limit` queries (or uses `scripts/evaluation/validate_retrieval.py` programmatically) and returns metrics (MAP, precision@k)
- [ ] Define a configurable acceptance threshold (e.g., `min_map_improvement` or absolute MAP floor). If new index fails gate, do NOT swap alias and write failure reason to audit log.
- [ ] Make validation run fast by using the existing `limit`/`debug_limit` options and a small sample (e.g., 50 queries) for automated checks.
- Outcome: basic quality gate prevents blind swaps that degrade retrieval performance

8) Alias swap & atomic cutover (15 minutes)
- [ ] Implement `swap_alias(old_index, new_index, alias_name)` which calls the `_aliases` API with both remove and add in the same request so the operation is atomic
- [ ] Before swap, write an audit entry (timestamp, model, old_index, new_index, metrics, git commit) to an `reindex_audit` index so swaps are historically recorded and visible in Kibana
- [ ] After swap, verify alias now points to new_index via `es.indices.get_alias(name=alias_name)` and confirm doc counts and mappings
