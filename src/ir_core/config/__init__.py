...
class Settings(BaseSettings):
    ...
    # enables boosted sparse retrieval using keywords_per_src.json
    USE_SRC_BOOSTS: bool = False
    USE_STOPWORD_FILTERING: bool = False  # strip global stopwords from queries
    # filter exact duplicates using duplicates.json
    USE_DUPLICATE_FILTERING: bool = False
    # penalize near-duplicates using near_duplicates.json
    USE_NEAR_DUP_PENALTY: bool = False
    ...
    def __init__(self, **data):
        super().__init__(**data)
        # Only log and validate if explicitly requested
        if os.getenv('VERBOSE_CONFIG', '').lower() in ('true', '1', 'yes'):
    ...
    def _log_configuration_details(self) -> None:
        ...
        logger.info(
            f"   • System Message File: {self.GENERATOR_SYSTEM_MESSAGE_FILE}")
        ...
        logger.info(
            f"   • Use Stopword Filtering: {self.USE_STOPWORD_FILTERING}")
        logger.info(
            f"   • Use Duplicate Filtering: {self.USE_DUPLICATE_FILTERING}")
        logger.info(
            f"   • Use Near Duplicate Penalty: {self.USE_NEAR_DUP_PENALTY}")
    ...
    def validate_configuration(self) -> list[str]:
        ...
        translation_enabled = getattr(
            self, 'translation', {}).get('enabled', False)
    ...
    def settings_customise_sources(cls, settings_cls, init_settings, env_settings, dotenv_settings, file_secret_settings):
        ...
        project_root = Path(
            __file__).parent.parent.parent.parent  # Adjust path to project root
        ...
        if settings_file.exists():
            ...
            try:
                ...
                pass
            yaml_data = cast(Dict[str, Any], OmegaConf.to_container(
                OmegaConf.load(settings_file), resolve=True))
...
__all__ = ["Settings", "settings",
           "enable_config_logging", "disable_config_logging"]
