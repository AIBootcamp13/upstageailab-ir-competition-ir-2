        ...
        for row in reader:
            ...
            if 'topk' in row and row['topk']:
                try:
                    ...
                    topk_ids = json.loads(row['topk']) if row['topk'].startswith('[') else [row['topk']]
...
def main():
    ...
    if len(sys.argv) < 4:
        print("Usage: python transform_submission.py <eval_file> <submission_file> <output_file>")
        print("Example: python transform_submission.py data/eval.jsonl outputs/submission.csv outputs/evaluation_logs.jsonl")
        sys.exit(1)
