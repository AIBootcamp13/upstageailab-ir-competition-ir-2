...
class SetupMenu(BaseMenuModule):
    ...
    def validate_setup(self) -> Dict[str, bool]:
        ...
        # Check if dependencies are installed (check for poetry.lock and pyproject.toml)
        poetry_lock = self.project_root / "poetry.lock"
        pyproject_toml = self.project_root / "pyproject.toml"
        results["dependencies_installed"] = poetry_lock.exists() and pyproject_toml.exists()
        ...
        try:
            ...
            results["services_available"] = es_running and redis_running
    ...
    def get_setup_instructions(self) -> str:
        ...
        if not validation["poetry_installed"]:
            instructions.append("❌ Poetry not installed - required for dependency management")
            instructions.append("   Install: curl -sSL https://install.python-poetry.org | python3 -")

        if not validation["dependencies_installed"]:
            instructions.append("⚠️  Dependencies not installed - run poetry install")
            instructions.append("   Run: poetry install")
