...
def run(query: str, index_name: str = "test", rerank_k: int = 5):
    ...
    try:
        ...
        print("You can start services with: ./scripts/run-local.sh start")
        print("You can index data with: PYTHONPATH=src poetry run python scripts/reindex.py data/documents.jsonl")
